<?php
/*
Plugin Name: Connections Slim Plus - Template
Plugin URI: http://www.connections-pro.com
Description: Connections Slim Plus - Template
Version: 1.0
...
if ( ! class_exists( 'CN_Slim_Plus' ) ) {
	...
	class CNT_Slim_Plus {
		...
		public static function enqueueCSS() {
			...
			wp_enqueue_style( 'cnt_' . self::$template->getSlug() . '-css', self::$url . "slim-plus$min.css", array(), self::$template->getVersion() );
		}
		...
		public static function enqueueJS() {
			...
			wp_enqueue_script( 'cnt_' . self::$template->getSlug() . '-js' , self::$url . "slim-plus$min.js", array( 'jquery-chosen-min' ), self::$template->getVersion(), TRUE );
		}
	 ...
	 */
	function Connections_Slim_Plus() {
	 ...
	 */
	add_action( 'plugins_loaded', 'Connections_Slim_plus', 11 );
}