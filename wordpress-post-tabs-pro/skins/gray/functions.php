<?php function wpts_pro_wp_init_gray($atts=array(),$data=array()) { 
    global $post,$wpts;
	if(is_singular() or $wpts['enable_everywhere'] == '1') { 
			...
			or (is_single() and ((!empty($enablewpts) and $enablewpts=='1') or $wpts['posts'] != '0'  ) ) or $wpts['enable_everywhere'] == '1' ) 
		{
			...
			$wpts_stylesheet='gray';
			$css='skins/'.$wpts_stylesheet.'/style.css';
			$js_folder='skins/'.$wpts_stylesheet.'/js';
			wp_enqueue_style( 'wpts_ui_css_gray', wpts_pro_plugin_url( $css ),false, WPTSPRO_VER, 'all'); 
...
function return_wpts_tab_script_gray( $wpts_count=0,$atts=array(),$data=array() ){
...
/*---------------------------- STYLE EDITOR ----------------------------*/
			if(isset($wpts_style[$wpts_stylesheet]) and is_array($wpts_style[$wpts_stylesheet])){
				...
				if((!empty($style_bg) and $style_bg!="#") or (!empty($style_active_bg) and $style_active_bg!="#") or (!empty($style_color) and $style_color!="#") or (!empty($style_active_color) and $style_active_color!="#")  or (!empty($style_hover_bg) and $style_hover_bg!="#") or (!empty($style_hover_color) and $style_hover_color!="#"))	{		
					...
					if(!empty($style_bg) and $style_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-default {background: '.$style_bg.';}'; else {if(!empty($style_active_bg) and $style_active_bg!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-default {background: #e6e6e6 url('.WPTSPRO_URLPATH.'skins/gray/images/ui-bg_glass_75_e6e6e6_1x400.png) 50% 50% repeat-x;}';}}	
					if(!empty($style_active_bg) and $style_active_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-active { background: '.$style_active_bg.';}'; else {if(!empty($style_bg) and $style_bg!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-active { background: #ffffff url('.WPTSPRO_URLPATH.'skins/gray/images/ui-bg_glass_65_ffffff_1x400.png) 50% 50% repeat-x;}';}}	
					if(!empty($style_hover_bg) and $style_hover_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-hover { background: '.$style_hover_bg.';}'; else {$script .= '#'.$tab_name.' > ul > li.ui-state-hover { background: #dadada url('.WPTSPRO_URLPATH.'skins/gray/images/ui-bg_glass_75_dadada_1x400.png) 50% 50% repeat-x;}';}	
					if(!empty($style_color) and $style_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-default .ui-tabs-anchor{ color: '.$style_color.';}'; else {if(!empty($style_active_color) and $style_active_color!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-default .ui-tabs-anchor{ color: #555555;}'; } }
					if(!empty($style_active_color) and $style_active_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-active .ui-tabs-anchor{ color:'.$style_active_color.';}'; else {if(!empty($style_color) and $style_color!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-active .ui-tabs-anchor{ color: #212121;}';}}	
					if(!empty($style_hover_color) and $style_hover_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-hover a { color: '.$style_hover_color.'!important;}'; else {$script .= '#'.$tab_name.' > ul > li.ui-state-hover a { color: #212121 !important;}';}
					$script .='</style>");';
				} // if empty check end 		
			}
/*------------------------- END STYLE EDITOR -------------------------*/
