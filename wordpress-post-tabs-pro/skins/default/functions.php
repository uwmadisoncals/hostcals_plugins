<?php function wpts_pro_wp_init_default($atts=array(),$data=array()) { 
    global $post,$wpts;
	if(is_singular() or $wpts['enable_everywhere'] == '1') { 
			...
			or (is_single() and ((!empty($enablewpts) and $enablewpts=='1') or $wpts['posts'] != '0'  ) ) or $wpts['enable_everywhere'] == '1' ) 
		{
			...
			$wpts_stylesheet='default';
			$css='skins/'.$wpts_stylesheet.'/style.css';
			$js_folder='skins/'.$wpts_stylesheet.'/js';
			wp_enqueue_style( 'wpts_ui_css_default', wpts_pro_plugin_url( $css ),false, WPTSPRO_VER, 'all'); 
...
function return_wpts_tab_script_default( $wpts_count=0,$atts=array(),$data=array() ){
...
/*---------------------------- STYLE EDITOR ----------------------------*/
			if(isset($wpts_style[$wpts_stylesheet]) and is_array($wpts_style[$wpts_stylesheet])){
				...
				if((!empty($style_bg) and $style_bg!="#") or (!empty($style_active_bg) and $style_active_bg!="#") or (!empty($style_color) and $style_color!="#") or (!empty($style_active_color) and $style_active_color!="#") or (!empty($style_hover_bg) and $style_hover_bg!="#") or (!empty($style_hover_color) and $style_hover_color!="#"))	{		
					...
					if(!empty($style_bg) and $style_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-default a {background: '.$style_bg.';}'; else {if(!empty($style_active_bg) and $style_active_bg!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-default a {background: #EFEFEF;}';}}	
					if(!empty($style_active_bg) and $style_active_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-active a { background: '.$style_active_bg.';}'; else {if(!empty($style_bg) and $style_bg!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-active { background: url('.WPTSPRO_URLPATH.'skins/default/images/arrowt.gif) no-repeat bottom center;} #'.$tab_name.' > ul > li.ui-state-active a {background:#5E5E5E}';}}	
					if(!empty($style_hover_bg) and $style_hover_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-hover a { background: '.$style_hover_bg.';}';
					if(!empty($style_color) and $style_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-default .ui-tabs-anchor{ color: '.$style_color.';}'; else {if(!empty($style_active_color) and $style_active_color!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-default .ui-tabs-anchor{ color: #5E5E5E;}'; } }
					if(!empty($style_active_color) and $style_active_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-active .ui-tabs-anchor{ color:'.$style_active_color.';}'; else {if(!empty($style_color) and $style_color!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-active .ui-tabs-anchor{ color: #FFF;}';}}		
					if(!empty($style_hover_color) and $style_hover_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-hover a { color: '.$style_hover_color.'!important;}'; 
					$script .='</style>");';
				} // if empty check end 		
			}
/*------------------------- END STYLE EDITOR -------------------------*/
