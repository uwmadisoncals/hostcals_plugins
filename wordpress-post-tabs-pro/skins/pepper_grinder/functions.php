<?php function wpts_pro_wp_init_pepper_grinder($atts=array(),$data=array()) { 
    global $post,$wpts;
	if(is_singular() or $wpts['enable_everywhere'] == '1') { 
			...
			or (is_single() and ((!empty($enablewpts) and $enablewpts=='1') or $wpts['posts'] != '0'  ) ) or $wpts['enable_everywhere'] == '1' ) 
		{
			...
			$wpts_stylesheet='pepper_grinder';
			$css='skins/'.$wpts_stylesheet.'/style.css';
			$js_folder='skins/'.$wpts_stylesheet.'/js';
			wp_enqueue_style( 'wpts_ui_css_pepper_grinder', wpts_pro_plugin_url( $css ),false, WPTSPRO_VER, 'all'); 
...
function return_wpts_tab_script_pepper_grinder( $wpts_count=0,$atts=array(),$data=array() ){
...
/*---------------------------- STYLE EDITOR ----------------------------*/
			if(isset($wpts_style[$wpts_stylesheet]) and is_array($wpts_style[$wpts_stylesheet])){
				...
				if((!empty($style_bg) and $style_bg!="#") or (!empty($style_active_bg) and $style_active_bg!="#") or (!empty($style_color) and $style_color!="#") or (!empty($style_active_color) and $style_active_color!="#")  or (!empty($style_hover_bg) and $style_hover_bg!="#") or (!empty($style_hover_color) and $style_hover_color!="#"))	{		
					...
					if(!empty($style_bg) and $style_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-default {background: '.$style_bg.';}'; else {if(!empty($style_active_bg) and $style_active_bg!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-default {background: #f8f7f6 url('.WPTSPRO_URLPATH.'skins/pepper_grinder/images/ui-bg_fine-grain_10_f8f7f6_60x60.png) 50% 50% repeat;}';}}	
					if(!empty($style_active_bg) and $style_active_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-active { background: '.$style_active_bg.';}'; else {if(!empty($style_bg) and $style_bg!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-active { background: #eceadf url('.WPTSPRO_URLPATH.'skins/pepper_grinder/images/ui-bg_fine-grain_15_eceadf_60x60.png) 50% 50% repeat;}';}}
					if(!empty($style_hover_bg) and $style_hover_bg!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-hover { background: '.$style_hover_bg.';}'; else {$script .= '#'.$tab_name.' > ul > li.ui-state-hover { background: #654b24 url('.WPTSPRO_URLPATH.'skins/pepper_grinder/images/ui-bg_fine-grain_65_654b24_60x60.png) 50% 50% repeat;}';}	
					if(!empty($style_color) and $style_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-default .ui-tabs-anchor{ color: '.$style_color.';}'; else {if(!empty($style_active_color) and $style_active_color!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-default .ui-tabs-anchor{ color: #654b24;}'; } }
					if(!empty($style_active_color) and $style_active_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-active .ui-tabs-anchor{ color:'.$style_active_color.';}'; else {if(!empty($style_color) and $style_color!="#"){$script .= '#'.$tab_name.' > ul > li.ui-state-active .ui-tabs-anchor{ color: #140f06;}';} }				
					if(!empty($style_hover_color) and $style_hover_color!="#") $script .= '#'.$tab_name.' > ul > li.ui-state-hover a { color: '.$style_hover_color.'!important;}'; else {$script .= '#'.$tab_name.' > ul > li.ui-state-hover a { color: #ffffff !important;}';}	
					$script .='</style>");';
				} // if empty check end 		
			}
/*------------------------- END STYLE EDITOR -------------------------*/
