...
//            media.gallery.defaults.size = 'thumbnail';
            ...
            media.gallery.defaults.folder_id = '';
            media.gallery.defaults.autoinsert = '0';
            ...
            this.update.apply( this, ['folder_id'] );
            
            if(id_folder != undefined){
                var oldfIds =  $el.find('.folder_id').val();
                var oldfIds_array = oldfIds.split(",").map(Number);
                
                    if(oldfIds != ''){
                        if(oldfIds_array.indexOf(id_folder) < 0){
                            $el.find('.folder_id').val(oldfIds+','+id_folder).change();
                        }
                    }else{
                        $el.find('.folder_id').val(id_folder).change();
                    }
            }
            
            this.update.apply( this, ['autoinsert'] );
            return this;
        }
    });
